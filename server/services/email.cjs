...
Generation Catalyst Team
     ...
     */
    async sendInvoiceEmail(email, clientName, invoice) {
        const baseUrl = process.env.FRONTEND_URL || 'http://localhost:5000';
    ...
    `;
        ...
        let lineItemsText = '';
        if (invoice.lineItems && invoice.lineItems.length > 0) {
            lineItemsText = '\nLine Items:\n' + invoice.lineItems.map(item => `  - ${item.description}: $${parseFloat(item.amount).toFixed(2)}`).join('\n') + '\n';
        }
        const text = `
Hello ${clientName},
...
Generation Catalyst Team
     ...
     */
    async sendPaymentReceiptEmail(email, clientName, payment) {
