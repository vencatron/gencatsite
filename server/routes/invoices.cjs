...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const { userId, invoiceNumber, amount, tax, description, lineItems, dueDate, sendEmail = true // Default to sending email
         } = req.body;
        ...
        if (sendEmail && targetUser.email) {
            try {
                ...
                emailSent = await email_1.emailService.sendInvoiceEmail(targetUser.email, targetUser.firstName || targetUser.username, {
                    invoiceNumber: (0, validation_1.sanitizeInput)(invoiceNumber),
                    amount: amount.toString(),
                    tax: taxNum.toString(),
                    totalAmount: totalAmount.toString(),
                    description: description ? (0, validation_1.sanitizeInput)(description) : undefined,
                    dueDate: new Date(dueDate),
...
router.post('/:id/send', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        // Get the user
        const user = await storage_1.storage.getUser(invoice.userId);
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
        if (!user.email) {
        ...
        // Send the email
        const emailSent = await email_1.emailService.sendInvoiceEmail(user.email, user.firstName || user.username, {
...
router.get('/all', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const invoices = await storage_1.storage.getAllInvoices();
